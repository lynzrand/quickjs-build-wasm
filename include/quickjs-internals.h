...
/// A rope-like structure to support efficient large string concatenation.
/// Rope is represented as a Red-Black tree.
...
struct JSString {
    ...
    uint8_t is_rope : 1; // 0 = regular string, 1 = rope
    ...
    union {
        ...
        uint16_t str16[0];
        JSStringRope rope[0]; // A rope-like structure to support efficient concat
        // Always has 1 element (the same as JSStringRope rope), but declared as 
        // as an array for consistency with other parts.
    } u;
};
...
typedef union JWST_PropertyInlineCache JWST_PropertyInlineCache;
...
typedef struct JSFunctionBytecode {
  ...
  JWST_PropertyInlineCache* ic; // Inline cache entries
  struct JWST_OptFuncImpl* opt; // Optimized function implementation
  struct {
...
// The holy rebuild function for resuming from a deoptimization!
JSValue jwst_rebuild_stack(
...
#pragma region Unexported functions

JSValue js_closure(JSContext* ctx, JSValue bfunc, JSVarRef** cur_var_refs,JSStackFrame* sf);
JSValue js_new_string_char(JSContext* ctx, uint16_t ch);
